...
## Usage
This repository includes a comprehensive set of examples demonstrating different integration patterns.

| Example | Pattern | Description |
| :--- | :--- | :--- |
| **[basic-client](./examples/basic-client)** | CLI Tool | One-shot execution: lists GPUs and checks status conditions. |
| **[streaming-daemon](./examples/streaming-daemon)** | Sidecar / Agent | Long-running process: uses gRPC interceptors and `Watch` streams. |
| **[controller-shim](./examples/controller-shim)** | Operator | **Informer Injection**: Driving a `controller-runtime` reconciler with local data. |

See [Examples](./examples) for detailed instructions on running these locally.

## Advanced Use: Informer Injection
For advanced use cases like Kubernetes Operators, this SDK supports **Informer Injection**. This pattern allows `controller-runtime` Managers to source device metrics directly from the local node via gRPC while continuing to watch standard Cluster resources from the API server.

See the [Controller Shim Example](./examples/controller-shim) for a complete guide on implementing this hybrid reconciliation pattern.
