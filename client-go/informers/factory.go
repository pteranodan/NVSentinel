...
type GPUInformerFactory struct {
	client GPU
...
func NewGPUInformerFactory(client GPU, resync time.Duration, indexers cache.Indexers) *GPUInformerFactory {
	if indexers == nil {
		indexers = cache.Indexers{}
	}

	return &GPUInformerFactory{
		client:   client,
		resync:   resync,
		indexers: indexers,
	}
...
func (f *GPUInformerFactory) GPUInformer() cache.SharedIndexInformer {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	f.informer = NewGPUInformer(f.client, GPUInformerOptions{
		ResyncPeriod: f.resync,
		Indexers:     f.indexers,
	}).Informer()
...
func (f *GPUInformerFactory) GPUInformerWithOptions(opts GPUInformerOptions) cache.SharedIndexInformer {
	return NewGPUInformer(f.client, opts).Informer()
}

func (f *GPUInformerFactory) Start(stopCh <-chan struct{}) {
	inf := f.GPUInformer()
	go inf.Run(stopCh)
}

func (f *GPUInformerFactory) WaitForCacheSync(stopCh <-chan struct{}) bool {
	inf := f.GPUInformer()
	return cache.WaitForCacheSync(stopCh, inf.HasSynced)
}
