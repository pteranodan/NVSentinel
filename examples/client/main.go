...
// Package main demonstrates resource discovery and status evaluation
// using the NVIDIA Device API clientset.
package main
...
func main() {
	...
	// Initialize the versioned clientset.
	...
	// List GPUs to discover node-local resources.
	...
	logger.Info("discovered GPUs", "count", len(gpus.Items))
	...
	for _, gpu := range gpus.Items {
		// Tip: use standard apimachinery helpers to check condition state.
