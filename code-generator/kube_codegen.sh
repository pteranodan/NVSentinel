...
function kube::codegen::gen_client() {
    ...
    "${GOBIN}/client-gen" \
        ...
        --output-dir "${out_dir}" \
        --output-pkg "${out_pkg}" \
        --clientset-name "${clientset_subdir}" \
    ...
    if [ "${watchable}" == "true" ]; then
        ...
        "${GOBIN}/lister-gen" \
            -v "${v}" \
            --go-header-file "${boilerplate}" \
            --output-dir "${out_dir}/${clientset_subdir}/${listers_subdir}" \
            --output-pkg "${out_pkg}/${clientset_subdir}/${listers_subdir}" \
            --plural-exceptions "${plural_exceptions}" \
            "${input_pkgs[@]}"
        ...
        ( kube::codegen::internal::grep -l --null \
            ...
            -r "${out_dir}/${clientset_subdir}/${informers_subdir}" \
            --include '*.go' \
            || true \
        ) | xargs -0 rm -f
        ...
        "${GOBIN}/informer-gen" \
            -v "${v}" \
            --go-header-file "${boilerplate}" \
            --output-dir "${out_dir}/${clientset_subdir}/${informers_subdir}" \
            --output-pkg "${out_pkg}/${clientset_subdir}/${informers_subdir}" \
            --versioned-clientset-package "${out_pkg}/${clientset_subdir}" \
            --listers-package "${out_pkg}/${clientset_subdir}/${listers_subdir}" \
            --plural-exceptions "${plural_exceptions}" \
            "${input_pkgs[@]}"
    fi
}
