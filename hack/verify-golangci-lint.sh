...
RESOLVED_REV=$(git rev-parse --short "${REVISION}" 2>/dev/null || echo "${REVISION}")
...
log "$PROGRESS" "Linting changes since '${RESOLVED_REV}' ..."

if ! golangci-lint run --new-from-rev="${RESOLVED_REV}" > "$LOG" 2>&1; then
    log "ERROR:" "GoLint failed: (Exit 1)"
    cat "$LOG"
